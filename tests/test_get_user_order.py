...
class TestUserOrder:
    ...
    def test_get_authorized_user_order(self, create_and_cleanup_user):
        ...
        requests.post(f'{URL}{Endpoint.order}', headers={'Authorization': token}, data=order)
        response_get = requests.get(f'{URL}{Endpoint.order}', headers={'Authorization': token})
    ...
    def test_get_unauthorised_user_order(self):
        response_get = requests.get(f'{URL}{Endpoint.order}')
